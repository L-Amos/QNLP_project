...
def training(model, train_dataset, val_dataset):
    for name, loss_fn in LOSS_METRICS.items():
        ...
        np.savetxt(f"results/{name}_train_costs.csv", np.mean(train_costs, axis=0), delimiter=',')
        np.savetxt(f"results/{name}_val_costs.csv", np.mean(val_costs, axis=0), delimiter=',')
        np.savetxt(f"results/{name}_train_corr.csv", np.mean(train_corr, axis=0), delimiter=',')
        np.savetxt(f"results/{name}_val_corr.csv", np.mean(val_corr, axis=0), delimiter=',')
